...
export function showProperties(obj) {
    ...
    if (layoutModeGet()) {
        ...
        $('#moduleProperty-inner-2').append("<div id='moduleProperty-header'>" + obj.objectType + "</div>");
        ...
        if (obj.subcircuitMutableProperties && obj.canShowInSubcircuit) {
            for (let attr in obj.subcircuitMutableProperties) {
                ...
                if (obj.subcircuitMutableProperties[attr].type == "number") {
                    ...
                    $('#moduleProperty-inner-2').append(s);
            ...
            if (!obj.labelDirectionFixed) {
                ...
                $('#moduleProperty-inner-2').append("<p>Label Direction: " + $(s).prop('outerHTML') + "</p>");
            }
        }
        ...
        $('#moduleProperty-2').show();
        $('#moduleProperty-inner-2').append("<div id='moduleProperty-header'>" + 'Project Properties' + '</div>');
        $('#moduleProperty-inner-2').append(`<p><span>Project:</span> <input id='projname' class='objectPropertyAttribute' type='text' autocomplete='off' name='setProjectName'  value='${getProjectName() || 'Untitled'}'></p>`);
        $('#moduleProperty-inner-2').append(`<p><span>Circuit:</span> <input id='circname' class='objectPropertyAttribute' type='text' autocomplete='off' name='changeCircuitName'  value='${globalScope.name || 'Untitled'}'></p>`);
        $('#moduleProperty-inner-2').append(`<p><span>Clock Time (ms):</span> <input class='objectPropertyAttribute' min='50' type='number' style='width:100px' step='10' name='changeClockTime'  value='${simulationArea.timePeriod}'></p>`);
        $('#moduleProperty-inner-2').append(`<p><span>Clock Enabled:</span> <label class='switch'> <input type='checkbox' ${['', 'checked'][simulationArea.clockEnabled + 0]} class='objectPropertyAttributeChecked' name='changeClockEnable' > <span class='slider'></span></label></p>`);
        $('#moduleProperty-inner-2').append(`<p><span>Lite Mode:</span> <label class='switch'> <input type='checkbox' ${['', 'checked'][lightMode + 0]} class='objectPropertyAttributeChecked' name='changeLightMode' > <span class='slider'></span> </label></p>`);
        $('#moduleProperty-inner-2').append("<p><button type='button' class='objectPropertyAttributeChecked btn btn-xs custom-btn--primary' name='toggleLayoutMode' >Edit Layout</button><button type='button' class='objectPropertyAttributeChecked btn btn-xs custom-btn--tertiary' name='deleteCurrentCircuit' >Delete Circuit</button> </p>");
        // $('#moduleProperty-inner').append("<p>  ");
    } else {
        $('#moduleProperty').show();
        $('#moduleProperty-2').show();
        ...
        if (!obj.fixedBitWidth) { $('#moduleProperty-inner-2').append(`<p><span>BitWidth:</span> <input class='objectPropertyAttribute' type='number'  name='newBitWidth' min='1' max='32' value=${obj.bitWidth}></p>`); }
        ...
        if (obj.changeInputSize) { $('#moduleProperty-inner-2').append(`<p><span>Input Size:</span> <input class='objectPropertyAttribute' type='number'  name='changeInputSize' min='2' max='10' value=${obj.inputSize}></p>`); }
        ...
        if (!obj.propagationDelayFixed) { $('#moduleProperty-inner-2').append(`<p><span>Delay:</span> <input class='objectPropertyAttribute' type='number'  name='changePropagationDelay' min='0' max='100000' value=${obj.propagationDelay}></p>`); }
        if (!obj.disableLabel)
        ...
        $('#moduleProperty-inner-2').append(`<p><span>Label:</span> <input class='objectPropertyAttribute' type='text'  name='setLabel' autocomplete='off'  value='${escapeHtml(obj.label)}'></p>`);
        ...
        if (!obj.labelDirectionFixed) {
            ...
            $('#moduleProperty-inner-2').append(`<p><span>Label Direction:</span> ${$(s).prop('outerHTML')}</p>`);
        }
        ...
        if (!obj.directionFixed) {
            ...
            $('#moduleProperty-inner-2').append(`<p><span>Direction:</span> ${$(s).prop('outerHTML')}</p>`);
        } else if (!obj.orientationFixed) {
            s = $(`${"<select class='objectPropertyAttribute' name='newDirection'>" + "<option value='RIGHT' "}${['', 'selected'][+(obj.direction === 'RIGHT')]} >RIGHT</option><option value='DOWN' ${['', 'selected'][+(obj.direction === 'DOWN')]} >DOWN</option><option value='LEFT' ` + `<option value='RIGHT'${['', 'selected'][+(obj.direction === 'LEFT')]} >LEFT</option><option value='UP' ` + `<option value='RIGHT'${['', 'selected'][+(obj.direction === 'UP')]} >UP</option>` + '</select>');
            $('#moduleProperty-inner').append(`<p><span>Orientation:</span> ${$(s).prop('outerHTML')}</p>`);
            $('#moduleProperty-inner-2').append(`<p><span>Orientation:</span> ${$(s).prop('outerHTML')}</p>`);
        }
    ...
    if (helplink) {
        ...
        $('#moduleProperty-inner-2').append('<p class="btn-parent"><button id="HelpButton" class="btn btn-primary btn-xs" type="button" >&#9432 Help</button></p>');
        $('#HelpButton').on('click', () => {
